...
export function LoginCard({ onRegisterClick, onLoginClick }: LoginCardProps) {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  const isValid = emailRegex.test(email) && password.trim().length > 0;
      ...
      <div className="flex w-[960px] h-[600px] overflow-hidden rounded-[20px] shadow-lg">
        <div className="flex flex-col items-center justify-center bg-[#4DD0E1] p-12 w-[320px]">
          <div className="text-white text-center mb-8">
            <div className="inline-flex items-center justify-center rounded-full bg-white/10 ring-1 ring-white/20 p-3 mb-5">
              <UserPlus className="w-7 h-7 text-white/90" />
            </div>
            <h2 className="text-2xl font-bold tracking-wide uppercase text-white/85">
              AINDA NÃO FEZ O CADASTRAR?
            </h2>
        ...
        <div className="flex flex-col items-center justify-center bg-white p-12 flex-1 min-w-0">
          <h1 className="text-5xl font-bold text-[#4DD0E1] mb-12 text-center w-full">
            BEM-VINDO
          </h1>
